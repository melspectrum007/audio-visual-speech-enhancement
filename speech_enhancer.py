...
def train(args):
	...
	train_samples = load_preprocessed_samples(train_preprocessed_blob_paths, max_samples=args.number_of_samples)
	val_samples = load_preprocessed_samples(val_preprocessed_blob_paths, max_samples=args.number_of_samples)

	train_video_samples, train_mixed_spectrograms, train_source_spectrograms = train_samples[:3]
	train_waveform = train_samples[-1]
	val_video_samples, val_mixed_spectrograms, val_source_spectrograms = val_samples[:3]
	val_waveform = val_samples[-1]
	...
	num_waveform_sapmles = num_frames / SPLIT * 4 * 160 * SPLIT
	...
	train_waveform = split_and_concat(train_waveform[:, :num_waveform_sapmles], axis=-1, split=SPLIT)
	val_waveform = split_and_concat(val_waveform[:, :num_waveform_sapmles], axis=-1, split=SPLIT)
	...
	train_waveform = one_hot_encoding(mu_law_quantization(train_waveform, 256, 32768), 256)
	val_waveform = one_hot_encoding(mu_law_quantization(val_waveform, 256, 32768), 256)
	...
	print 'waveform shape:', train_waveform.shape
	...
	network.train(
		train_mixed_spectrograms, train_video_samples, train_source_spectrograms, train_waveform,
		val_mixed_spectrograms, val_video_samples, val_source_spectrograms, val_waveform
	)
