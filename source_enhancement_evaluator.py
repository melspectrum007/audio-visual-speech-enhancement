...
def pesq(pesq_bin_path, source_file_path, enhanced_file_path):
	...
	random_unique_prefix = str(uuid.uuid4())
	temp_source_path = os.path.join(temp_dir, random_unique_prefix + 'source.wav')
	temp_enhanced_path = os.path.join(temp_dir, random_unique_prefix + 'enhanced.wav')
...
def evaluate(enhancement_dir_path, pesq_bin_path):
	...
	for sample_dir_name in sample_dir_names:
		...
		mixture_file_path = os.path.join(sample_dir_path, 'mixed.wav')
